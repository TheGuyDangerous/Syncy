...
>
> **Implemented so far:** the engine core (SHA‑256 content identifiers, content‑defined chunking, SQLite metadata store); a folder scanner with rename/move/delete‑aware index diffing and a live filesystem monitor; per‑device **Ed25519 identity with mutual TLS**; an authenticated **QUIC transport**; the **DeltaSync Protocol**; and a **reconciliation session that syncs real files between two peers** over the network — delta block reuse, dedup, and block + whole‑file integrity verification, all unit‑ and integration‑tested and green on Windows, macOS and Linux CI. The **desktop app** (Tauri) shell builds into native installers. **Next up:** the sync‑engine orchestration (conflicts, versioning, offline queue), device discovery, and wiring the engine into the desktop and mobile clients.
